...
namespace MedScanRx.Controllers.App
{
	...
	public class PrescriptionController : Controller
	{
		...
		public async Task<IActionResult> PrescriptionWithAlerts(int prescriptionId)
		{
			try
			{
				var allPrescriptionsWithAlerts = await _bll.GetPrescriptionWithActiveAlerts(prescriptionId).ConfigureAwait(false);
		...
		[HttpPut]
		[Route("{prescriptionAlertId}")]
		public async Task<IActionResult> TakeMedicine(int prescriptionAlertId)
		{
			try
			{
				bool wasTaken = await _bll.TakeMedicine(prescriptionAlertId);

				return Ok();
			} catch (Exception ex)
			{
				return StatusCode(500, new { errorMessage = "Server error trying to mark a medicine as taken" });
